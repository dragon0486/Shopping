        ...
        <div class="main">
            <div class="course-info">
                ...
                <div class="wrap-right">
                    ...
                    <div class="buy">
                        <div class="buy-btn">
                            <button class="buy-now" @click="buyCourse">立即购买</button>
                    ...
                    <div class="tab-item" v-if="tabIndex==2">
                        ...
                        <div class="chapter-item" v-for="chapter in course_chapters">
                            ...
                            <ul class="section-list">
                                <li class="section-item" v-for="section in chapter.coursesections">
...
<script>
    ...
    export default {
      ...
      methods: {
        ...
        get_course_data() {
            // ajax请求课程信息
            this.$axios.get(`${this.$settings.BASE_HTTP}/course/${this.course_id}/`).then(response => {
                window.console.log(response.data);
                this.course_info = response.data;
            }).catch(() => {
                this.$message({
                    message: "对不起，课程数据出错！请联系客服工作人员！"
                });
            })
        },
        get_chapter() {
            // 获取当前课程对应的章节课时信息
            this.$axios.get(`${this.$settings.BASE_HTTP}/course/chapters/`, {
                params: {
                    "course": this.course_id,
                }
            }).then(response => {
                this.course_chapters = response.data.results;
            }).catch(() => {
                this.$message({
                    message: "对不起，章节数据出错！请联系客服工作人员！"
                });
            })
        },
        onPlayerPlay() {
        ...
        buyCourse () {
            let token = sessionStorage.token||this.$store.state.token;
            if (!token) {
                this.$message({
                    message: '请先登录!'
                });
            ...
            this.$axios({
                url: this.$settings.BASE_HTTP + '/order/',
                data: {
                    subject: this.course_info.name,
                    money: this.course_info.price,
                },
                method: 'post',
                headers: {
                ...
                this.order_info = response.data.results;
                window.console.log(this.order_info);
                window.open(this.order_info.trade_url)
            });


            this.$alert('是否完成支付？', '订单', {
                callback: () => {
                    // 拿订单号校验订单状态
                    this.$message({
                        message: this.order_info.trade_no
                    })
                }
            })
        },
