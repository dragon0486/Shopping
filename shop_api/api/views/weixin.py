...
def index(request):
    obj = models.UserInfo2.objects.get(id=1)
    return render(request,'index_wx.html',{'obj':obj})
...
def login(request):
    ...
    if request.method == "POST":
        ...
        obj = models.UserInfo2.objects.filter(username=user, password=pwd).first()
        if obj:
...
def bind_qcode(request):
    ...
    try:
        ...
        access_url = access_url.format(
            ...
            redirect_uri=settings.WECHAT_CONFIG["redirect_uri"], # 用户扫描后微信会向'http://47.93.4.198/test/'发请求,
    ...
    return JsonResponse(ret)        # 前端根据access_url通过jquery.qrcode生成二维码
...
def callback(request):
    ...
    res = requests.get(
        ...
        params={
            "appid": 'wx5acb9aa36cc1c77f',
            "secret": '36e1b1bd5849d400e06e39734caa819c',
            "code": code,
    ...
    if openid:
        models.UserInfo2.objects.filter(uid=state).update(wx_id=openid)
...
def sendmsg(request):
    ...
    openid = models.UserInfo2.objects.get(id=1).wx_id
    ...
    def send_template_msg():
        ...
        res = requests.post(
            ...
            json={
                "touser": openid,
                "template_id": 'UVhnP-WU6TUUSRCsxeLrRI95yV3JdV_MkMUHRpYnXmI',
                "data": {
                    "first": {
                        "value": "云姐",
                        "color": "#173177"
